        const ALL_SEGS = [
            { key: 'seg1', label: '1-2鑺?, time: '09:00-10:20' },
            { key: 'seg2', label: '3-4鑺?, time: '10:40-12:00' },
            { key: 'seg3', label: '5-6鑺?, time: '12:30-13:50' },
            { key: 'seg4', label: '7-8鑺?, time: '14:00-15:20' },
            { key: 'seg5', label: '9-10鑺?, time: '15:30-16:50' },
            { key: 'seg6', label: '11-12鑺?, time: '17:00-18:20' },
            { key: 'seg7', label: '13-14鑺?, time: '19:00-20:20' },
            { key: 'seg8', label: '15-16鑺?, time: '20:30-21:50' },
        ];
        ...
        let DAYS = ['鍛ㄤ竴', '鍛ㄤ簩', '鍛ㄤ笁', '鍛ㄥ洓', '鍛ㄤ簲', '鍛ㄥ叚', '鍛ㄦ棩'];
        ...
        function addStudentFromInput() {
            ...
            if (!name) return;
            if (/^\d+$/.test(name)) { toast('濮撳悕涓嶈兘涓虹函鏁板瓧', 'warning'); return; }
        ...
        function renderStudentList() {
            ...
            document.getElementById('student-count').textContent = `鍏?${students.length} 浜篳;
            l.innerHTML = '';
            students.forEach(s => {
      ...
      <div class="student-info"><div class="name">${s.name}</div><div class="slots">绌洪棽鎺掔彮: ${activeFree.length} &nbsp;路&nbsp; 宸叉帓: ${ac}</div></div></div>
      <button class="gender-btn ${s.gender === 'M' ? 'male' : 'female'}" onclick="toggleGender(${s.id})">${s.gender === 'M' ? '鐢? : '濂?}</button>`;
                l.appendChild(d);
            });
        }
        ...
        function renderPool() {
            ...
            document.getElementById('pool-count').textContent = `${active.length} 浜哄弬涓巂;
            p.innerHTML = '';
            ...
            active.forEach(s => {
                ...
                let sc = 'ok', st = `${c}鐝璥;
                if (hc) { sc = 'conflict'; st = '鍐茬獊'; } else if (c >= max && max > 0) { sc = 'overload'; }
        ...
        function renderScheduleTable() {
            ...
            th.innerHTML = `<tr><th>鏃堕棿 / 鏄熸湡</th>${ad.map(d => `<th>${DAYS[DAY_KEYS.indexOf(d)]}</th>`).join('')}</tr>`;
        ...
        function renderCell(k, c, id) {
            ...
            const head = `<div class="cell-header">${hc ? `<span class="cell-conflict-label">鍐茬獊</span>` : `<span class="cell-id">#${String(id).padStart(3, '0')}</span>`}<button class="split-btn" onclick="${c.split ? `unsplitCell('${k}')` : `splitCell('${k}')`}">${c.split ? '鍚堝苟' : '鍗曞弻鍛?}</button></div>`;
            if (c.split) return `<div class="sched-cell${cc}" data-key="${k}">${head}<div class="slots-split"><div class="split-col"><div class="split-week-label">鍗曞懆</div>${c.odd.map((sl, i) => renderSlotRow(sl, k, i, 'odd')).join('')}</div><div class="split-col"><div class="split-week-label">鍙屽懆</div>${c.even.map((sl, i) => renderSlotRow(sl, k, i, 'even')).join('')}</div></div></div>`;
        ...
        function renderSlotRow(sl, k, idx, w = null) {
            ...
            const rb = sl.name ? `<button class="remove-btn" onclick="removeFromSlot('${k}',${idx}${w ? `,'${w}'` : ''})" title="绉婚櫎"><span class="material-symbols-outlined">close</span></button>` : '';
            return `<div class="slot-row ${sl.gender === 'M' ? 'male-slot' : 'female-slot'}" data-key="${k}" data-idx="${idx}" ${wa} ondragover="event.preventDefault();this.classList.add('drag-over-slot')" ondragleave="this.classList.remove('drag-over-slot')" ondrop="onDrop(event,this,'${k}',${idx},'${w || ''}')" ondragstart="onSlotDragStart(event,this)">
    <span class="material-symbols-outlined slot-icon ${sl.gender === 'M' ? 'male' : 'female'}">${sl.gender === 'M' ? 'male' : 'female'}</span><span class="slot-name ${sl.name ? '' : 'empty'}">${sl.name || '寰呭垎閰?}</span>${ci}${rb}</div>`;
        }

        function checkCellConflict(c) { const f = (s) => s.some(sl => sl.name && checkSlotConflict(sl)); return c.split ? (f(c.odd) || f(c.even)) : f(c.slots); }
        function checkSlotConflict(sl) { const s = students.find(x => x.name === sl.name); return s && s.gender === 'F' && sl.gender === 'M'; }
        ...
        function onSlotDragStart(e, el) {
            const n = el.querySelector('.slot-name')?.textContent; if (!n || n === '寰呭垎閰?) { e.preventDefault(); return; }
        ...
        function onDrop(e, el, k, idx, w) {
            ...
            if (!s) { toast('璇ュ鐢熸湭鍙備笌鎺掔彮', 'warn'); return; }
        ...
        function executeScheduling(isContinue) {
            const active = students.filter(s => s.active);
            if (!active.length) { toast('璇峰厛娣诲姞骞跺嬀閫夊鐢?, 'warn'); return; }
            ...
            renderAll(); toast(isContinue ? '澧為噺濉厖瀹屾垚' : '鎺掔彮瀹屾垚', 'success');
        }
      ...
      ${s.label} (${s.time})
        ...
        function saveSettings() {
            ...
            buildEmptySchedule(); renderAll(); closeSettings(); toast('璁剧疆宸叉洿鏂?, 'success');
        }

        function openFreeTime(name) {
            const s = students.find(x => x.name === name);
            if (!s) return;
            document.getElementById('freetime-title').textContent = `${s.name} 鐨勭┖闂茶鍥綻;
            const th = document.getElementById('freetime-head');
            th.innerHTML = `<tr><th style="width:100px;text-align:center;padding:12px 0;">鏃堕棿甯?/th>${DAYS.map(d => `<th style="padding:12px 0;">${d}</th>`).join('')}</tr>`;
            ...
            ALL_SEGS.forEach(seg => {
                ...
                DAY_KEYS.forEach(day => {
                    ...
                    const isFree = !!fs;
                    let weekStr = fs ? (fs.split('|')[1] || '1-18鍛?) : '';
        ...
        function openManualFreetime(name) {
            ...
            document.getElementById('manual-freetime-title').textContent = `涓?${name} 鎵嬪姩鍒嗛厤绌洪棽鏃堕棿`;
        ...
        function stringifyWeeks() {
            ...
            if (weekSelection.size === 18) return '1-18鍛?;
        ...
        function parseWeeks(str) {
            let set = new Set();
            if (!str) return set;
            if (str === '1-18鍛?) { for (let i = 1; i <= 18; i++) set.add(i); return set; }
            let s = str.replace('鍛?, '');
            let parts = s.split(',');
            for (let p of parts) {
                if (p.includes('-')) {
        ...
        function updateWeekPreview() {
            document.getElementById('week-selector-preview').textContent = stringifyWeeks() || '鏈€夋嫨';
        }
        ...
        function toggleManualSlot(td, key) {
            ...
            if (manualFreeSlotsMap.has(key)) {
                ...
                initWeekGrid();
                weekSelection = parseWeeks('1-18鍛?); // Default
                updateWeekGridUI();
        ...
        function confirmManualFreetime() {
            ...
            closeManualFreetime();
            toast(`鎴愬姛娣诲姞瀛︾敓: ${manualStudentName}`, 'success');
        }
        ...
        function processFile(file) {
            ...
            reader.onload = e => {
                ...
                // Find header
                let hr = -1;
                for (let r = 0; r < Math.min(js.length, 5); r++) { if (js[r].some(c => typeof c === 'string' && (c.includes('鍛ㄤ竴') || c.includes('鏄熸湡')))) { hr = r; break; } }
                if (hr === -1) { toast('瑙ｆ瀽琛ㄥご澶辫触', 'error'); return; }
                ...
                closeImport(); toast('瑙ｆ瀽Excel瀹屾瘯', 'success');
            };
        ...
        function renderFooter() {
            ...
            document.getElementById('stat-male').textContent = `鐢风敓浜烘暟: ${m}`; document.getElementById('stat-female').textContent = `濂崇敓浜烘暟: ${f}`;
            document.getElementById('stat-filled').textContent = `宸叉帓鐝? ${t ? Math.round(fl / t * 100) : 0}%`;
        }

        function exportCSV() {
            const ad = getActiveDays();
            const rs = [['鏃堕棿娈?].concat(ad.map(d => DAYS[DAY_KEYS.indexOf(d)]))];
            getActiveSegs().forEach(seg => {
                const mr = [seg.label + '(鐢?'], fr = [seg.label + '(濂?'];
                ad.forEach(d => {
                    ...
                    if (c && c.split) {
                        ...
                        mr.push(`鍗?${oM} 鍙?${eM}`); fr.push(`鍗?${oF} 鍙?${eF}`);
                    } else if (c) {
            ...
            a.download = '鎺掔彮琛?csv'; a.click();
        }
        function saveSchedule() { localStorage.setItem('schedule_data', JSON.stringify({ students, schedule, activeSegKeys, activeDayKeys, slotMin, slotMax })); toast('淇濆瓨鎴愬姛'); }
        function tryRestore() { const s = localStorage.getItem('schedule_data'); if (s) { try { const p = JSON.parse(s); if (p.activeSegKeys) activeSegKeys = p.activeSegKeys; if (p.activeDayKeys) activeDayKeys = p.activeDayKeys; if (p.slotMin !== undefined) slotMin = p.slotMin; if (p.slotMax !== undefined) slotMax = p.slotMax; students = p.students || []; Object.assign(schedule, p.schedule || {}); return true; } catch { return false; } } return false; }
        ...
        window.addEventListener('beforeunload', e => {
            const current = JSON.stringify({ students, schedule, activeSegKeys, activeDayKeys, slotMin, slotMax });
